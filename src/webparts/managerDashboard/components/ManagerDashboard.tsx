...
function generateAllRfisPdf(rfis: IRfi[], projects: IProject[]): void {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const doc: any = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });
    const pw = 297; const ph = 210; const ml = 10; const mr = 10; const tw = pw - ml - mr;
    const hdrH = 28;
    doc.setFillColor(17, 20, 24); doc.rect(0, 0, pw, hdrH, 'F');
    if (IMG_LOGO_PDF) doc.addImage(IMG_LOGO_PDF, 'PNG', ml, 4, 32, 20);
    doc.setDrawColor(42, 158, 42); doc.setLineWidth(0.4); doc.line(ml + 36, 6, ml + 36, hdrH - 6);
    doc.setTextColor(255, 255, 255); doc.setFontSize(14); doc.setFont('helvetica', 'bold');
    doc.text('RFI LIST', ml + 41, 13);
    doc.setTextColor(138, 155, 176); doc.setFontSize(8);
    doc.text('Generated: ' + new Date().toLocaleDateString('en-AU') + '  |  ' + rfis.length + ' RFIs', ml + 41, 22);
    let y = hdrH + 5;
    const cols = [
    ...
    const pages = doc.internal.getNumberOfPages();
    for (let i = 1; i <= pages; i++) { doc.setPage(i); doc.setFontSize(7); doc.setTextColor(138, 155, 176); doc.text('3 Edge Design', ml, ph - 5); doc.text('Page ' + i + ' of ' + pages, pw - mr, ph - 5, { align: 'right' }); }
...
function generateAllEwosPdf(ewos: IProject[], projects: IProject[]): void {
  try {
    ...
    doc.setDrawColor(42, 158, 42); doc.setLineWidth(0.4); doc.line(ml + 36, 6, ml + 36, hdrH - 6);
    doc.setTextColor(255, 255, 255); doc.setFontSize(14); doc.setFont('helvetica', 'bold');
    doc.text('EWO LIST', ml + 41, 13);
    doc.setTextColor(138, 155, 176); doc.setFontSize(8);
    doc.text('Generated: ' + new Date().toLocaleDateString('en-AU') + '  |  ' + ewos.length + ' EWOs', ml + 41, 22);
    let y = hdrH + 5;
    const cols = [
